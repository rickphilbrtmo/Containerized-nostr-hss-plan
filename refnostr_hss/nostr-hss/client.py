...
def main():
    ...
    parser.add_argument("--serve",        action="store_true", help="Start AS node and listen for PNRs only (no SNR registration)")
    ...
    if args.serve:
        log.info(f"Serve mode — listening for PNRs on {args.origin_host} (no SNR registration)")
    else:
        subs = []
        if args.from_file:
            ...
            log.error("Use --npub + --pallets, --from-file, or --serve")
            node.stop()
            sys.exit(1)
